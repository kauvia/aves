...
class SpriteUpdateSys {
	...
	updateAnimatedSprites(objArr, i) {
		if (objArr[i].Stats.health <= 0) {
			...
			this.stage.addChild(objArr[i].Sprite.death);
			objArr[i].Sprite.death.animationSpeed = 0.1;
			objArr[i].Sprite.death.loop = false;
			objArr[i].Sprite.death.play();
		} else {
				...
				if (objArr[i].Movement.attacking) {
					...
					if (
						...
						if (
							...
							objArr[i].Weapon.tick++;
							
							if (objArr[i].Weapon.tick>=5){
								objArr[i].Weapon.tick = 0;
